...
namespace Albacore.ViVe.NativeStructs
{
    ...
    public struct RTL_FEATURE_CONFIGURATION
    {
        ...
        {
            get => (RTL_FEATURE_CONFIGURATION_PRIORITY)(CompactState & 0xF);
            set
            {
                if ((uint)value > 15)
        ...
        {
            get => (RTL_FEATURE_ENABLED_STATE)((CompactState & 0x30) >> 4);
            set
            {
                if ((uint)value > 2)
        ...
        {
            get => ((CompactState & 0x40) >> 6) == 1;
            set => CompactState = (CompactState & 0xFFFFFFBF) | ((value ? (uint)1 : 0) << 6);
        }

        public bool HasSubscriptions
        {
            get => ((CompactState & 0x80) >> 7) == 1;
            set => CompactState = (CompactState & 0xFFFFFF7F) | ((value ? (uint)1 : 0) << 7);
        }

        public uint Variant
        {
            get => (CompactState & 0x3F00) >> 8;
            set
            {
                if (value > 63)
        ...
        {
            get => (RTL_FEATURE_VARIANT_PAYLOAD_KIND)((CompactState & 0xC000) >> 14);
            set
            {
                if ((uint)value > 3)
    ...
    public struct RTL_FEATURE_CONFIGURATION_UPDATE
    {
        ...
        public RTL_FEATURE_CONFIGURATION_OPERATION Operation
        {
            ...
            {
                if ((uint)value > 7)
                    throw new FeaturePropertyOverflowException("Operation", 7);
                _operation = value;
            }
        }
    }
